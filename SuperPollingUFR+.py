...
API_TOKEN = "7777111228:AAFJNsgzYXjfd1fNXIbffB8IhNBnwC5Nj6g"
ADMIN_ID = 5873723609
...
async def install_lib(message: types.Message):
    ...
    if len(args) < 2:
        await message.answer("❗️ Iltimos, o‘rnatiladigan kutubxona nomini kiriting.\nMisol: /install aiogram")
        return
    package = args[1].strip()
    await message.answer(f"🔄 <code>{package}</code> kutubxonasi o‘rnatilmoqda...")
    ...
    if proc.returncode == 0:
        ...
        await message.answer(f"✅ <code>{package}</code> muvaffaqiyatli o‘rnatildi.\n\n<pre>{output[:1000]}</pre>")
    else:
...
async def uninstall_lib(message: types.Message):
    ...
    args = message.text.strip().split(maxsplit=1)
    if len(args) < 2:
        await message.answer("❗️ Iltimos, o‘chiradigan kutubxona nomini kiriting.\nMisol: /uninstall aiogram")
        return
    package = args[1].strip()
    await message.answer(f"🗑️ <code>{package}</code> kutubxonasi o‘chirilyapti...")
    proc = await asyncio.create_subprocess_shell(
    ...
    if proc.returncode == 0:
        output = stdout.decode().strip()
        await message.answer(f"✅ <code>{package}</code> muvaffaqiyatli o‘chirildi.\n\n<pre>{output[:1000]}</pre>")
    else:
        error = stderr.decode().strip()
        await message.answer(f"❌ Xatolik yuz berdi:\n<pre>{error[:1000]}</pre>")
...
async def list_installed_packages(message: types.Message):
    ...
    await message.answer("📦 O‘rnatilgan kutubxonalar ro‘yxati olinmoqda...")
    proc = await asyncio.create_subprocess_shell(
        "pip list",
        stdout=asyncio.subprocess.PIPE,
        stderr=asyncio.subprocess.PIPE
    ...
    if proc.returncode == 0:
        ...
        if len(output) > 4000:
            ...
            with open(path, "w") as f:
                f.write(output)
            await message.answer_document(FSInputFile(path), caption="📦 Kutubxonalar ro‘yxati")
            os.remove(path)
        else:
...
async def freeze_requirements(message: types.Message):
    if message.from_user.id != ADMIN_ID:
        return await message.answer("❌ Sizda bu buyruqni bajarish huquqi yo‘q.")
    await message.answer("📝 requirements.txt fayli tayyorlanmoqda...")
    proc = await asyncio.create_subprocess_shell(
        "pip freeze",
        stdout=asyncio.subprocess.PIPE,
        stderr=asyncio.subprocess.PIPE
    ...
    stdout, stderr = await proc.communicate()
    if proc.returncode == 0:
        requirements_text = stdout.decode()
        filename = "requirements.txt"
        with open(filename, "w") as f:
            f.write(requirements_text)
        await message.answer_document(FSInputFile(filename), caption="📝 requirements.txt fayli")
        os.remove(filename)
    else:
        error = stderr.decode().strip()
        await message.answer(f"❌ Xatolik:\n<pre>{error}</pre>")
